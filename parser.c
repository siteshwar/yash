...
typedef enum tokentype_T {
    ...
    TT_GREATERGREATER, TT_GREATERPIPE, TT_GREATERAMP, TT_GREATERLPAREN,
    /* reserved words */
...
skip_blanks:
	...
	case L'>':
		...
		case L'>': ps->tokentype = TT_GREATERGREATER; index++; break;
...
bool psubstitute_alias(parsestate_T *ps, substaliasflags_T flags)
{
    ...
    if (substituted) {
	/* The current token has been substituted and is no longer valid. */
	wordfree(ps->token);
	ps->token = NULL;
	ps->tokentype = TT_UNKNOWN;
	ps->next_index = ps->index;
...
and_or_T *parse_command_list(parsestate_T *ps, bool toeol)
{
	...
	if (ps->tokentype != TT_AMP && ps->tokentype != TT_SEMICOLON) {
	    ...
	    // Trigger next_token in the next iteration
	    ps->tokentype = TT_UNKNOWN;
	}
    }
    if (!toeol)
...
assign_T *tryparse_assignment(parsestate_T *ps)
{
    ...
    wmemmove(ps->token->wu_string, &nameend[1], wcslen(&nameend[1]) + 1);
    if (ps->token->wu_string[0] == L'\0') {
	wordunit_T *wu = ps->token->next;
	wordunitfree(ps->token);
	ps->token = wu;
    }
...
{
    tokentype_T start, end;

    switch (type) {
	case CT_GROUP:     start = TT_LBRACE, end = TT_RBRACE;  break;
	case CT_SUBSHELL:  start = TT_LPAREN, end = TT_RPAREN;  break;
	default:           assert(false);
    }
    assert(ps->tokentype == start);
    next_token(ps);
	...
	serror(ps, Ngt("commands are missing between `%ls' and `%ls'"),
		type == CT_GROUP ? L"{" : L"(",  // TODO should have a better
		type == CT_GROUP ? L"}" : L")"); // converter function
    if (ps->tokentype != end)
	next_token(ps);
    else
	print_errmsg_token_missing(ps, type == CT_GROUP ? L"}" : L")");
    return result;
}
